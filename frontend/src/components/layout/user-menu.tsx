...
export function UserMenu() {
  ...
  const canSwitchViews = isExecutive(user);
  const isInMemberView = viewMode === "member";
      ...
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuLabel className="font-normal">
          ...
          {user.role && (
            <p className="mt-0.5 truncate text-xs text-muted-foreground">
              {isInMemberView ? "Viewing as: Member" : user.role.name}
            </p>
          )}
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        {canSwitchViews && (
          <>
            <DropdownMenuItem
              onClick={() => {
                setViewMode(isInMemberView ? "executive" : "member");
                router.push("/dashboard");
              }}
            >
              <ArrowLeftRight />
              {isInMemberView ? `Switch to ${user.role?.name ?? "Executive"} view` : "Switch to Member view"}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
          </>
        )}
