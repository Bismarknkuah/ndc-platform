...
export default function DepartmentTeamPage() {
  return (
    <Suspense fallback={<Skeleton className="h-64" />}>
      <DepartmentTeamPageContent />
    </Suspense>
  );
}

function DepartmentTeamPageContent() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const searchParams = useSearchParams();
  const user = useAuthStore((s) => s.user);

  const deepLinkedUnitId = searchParams.get("unit");

  const { data: deepLinkedUnit } = useQuery({
    queryKey: ["unit", deepLinkedUnitId],
    queryFn: () => hierarchyApi.getUnit(deepLinkedUnitId!),
    enabled: !!deepLinkedUnitId,
  });

  // manualUnit is only set once the person actually touches the
  // picker - until then, the displayed unit is derived from a ?unit=
  // deep link (from the dashboard's "Open Office" card, which names
  // the exact unit the department assignment is actually at) or
  // otherwise the caller's own home unit. Deriving this instead of
  // syncing it into state via an effect avoids an extra render pass.
  const [manualUnit, setManualUnit] = useState<{ id: string; name: string } | null>(null);
  const fallbackUnit = user?.organizational_unit
  ...
  const unit =
    ...
    (deepLinkedUnitId
      ? deepLinkedUnit
        ? { id: deepLinkedUnit.id, name: deepLinkedUnit.name }
        : null
      : fallbackUnit);
  const setUnit = setManualUnit;
