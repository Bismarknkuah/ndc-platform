...
export default function DashboardPage() {
  ...
  const { profile } = data;
  // The backend always returns the real data the caller's actual role
  // entitles them to - it has no concept of this frontend-only view
  // toggle. Suppressing these fields here, rather than at the API
  // layer, is what makes "Member view" a genuine no-signout switch:
  // the same account, same request, just a different lens on the
  // response.
  const inMemberView = viewMode === "member";
  ...
  return (
    <div className="flex flex-col gap-6">
      {inMemberView && (
        <div className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/[0.04] px-4 py-2 text-sm">
          <span>
            You&apos;re viewing your personal Member portal. Your{" "}
            {profile.role?.name ?? "executive"} tools are one click away.
          </span>
        </div>
      )}
      <div>
      ...
      {jurisdictionSummary ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <JurisdictionSummaryCard summary={jurisdictionSummary} />
          <ExecutiveAiPanel jurisdictionSummary={jurisdictionSummary} />
        </div>
      ) : (
      ...
      <StaggerContainer className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StaggerItem>
          {teamsLed ? (
            <StatCard icon={Users} label="Teams led" value={teamsLed.length} />
          ) : (
          ...
          <Card>
            <CardHeader className="flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">
                {financeSummary.organizational_unit.name}
              </CardTitle>
              <div className="flex items-center gap-1 text-sm">
                <Coins className="size-4 text-primary" />
                <span className="font-display font-semibold">
                  GHS {financeSummary.net_balance}
                </span>
              </div>
            </CardHeader>
            <CardContent>
              <FinanceBreakdownChart categories={financeSummary.by_category} />
            </CardContent>
          </Card>
        </section>
      )}
