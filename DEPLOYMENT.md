...
The login page shows one-click "try a demo account" buttons for seven
identities: a genuine Superadmin, and one role-scoped account per real
jurisdiction level (National Chairman, Regional Chairman, District
Co-ordinator, Constituency Chairman, Branch Secretary, Ordinary Member).
**This is a deliberate product decision, not an oversight**: these
buttons are visible to anyone who visits the site, including on the real
production deployment, with full understanding that this means anyone
can assume any of these seven identities with zero credentials.

**The Superadmin account is a materially larger exposure than the other
six**, and is called out separately for that reason: it carries
`is_superadmin=True`, which bypasses *every* permission check platform
-wide, not just its own Role's permission list. The other six are real
accounts too, but each is still bounded by an actual Role's actual
permission list and organizational-unit scope - a Regional Chairman
demo account can't touch National-level settings, for instance. All six
jurisdiction roles carry the same broad, multi-feature permission set
(hierarchy, finance, elections, membership, messaging - not just one
narrow tag each), including District Co-ordinator, which initially
shipped with a narrower coordination-only set reflecting Article 17's
literal description of the District Co-ordinating Committee as a body
that coordinates constituencies rather than commands them - broadened
to match the other levels by explicit request, a deliberate override of
that constitutional reading rather than an oversight. If constitutional
...
given that choice:
- Only one of the seven demo accounts (`demo.superadmin@ndc.example`)
  carries `is_superadmin` - the other six only have the real permissions
  their seeded `Role` grants, same as any actual member with that role.
