...
from apps.analytics.permissions import (
    can_access_ground_intelligence_for_unit,
    can_view_analytics,
)
...
class GroundIntelligenceView(APIView):
    ...
    Real, aggregated complaint/welfare/report data for a unit and its
    whole subtree - the actual ground situation, not a guess. Two
    tiers: the party's top leadership (Flagbearer, National Chairman,
    General Secretary) can reach any unit; every other real executive
    can reach it too, scoped to their own jurisdiction only (see
    can_access_ground_intelligence_for_unit).
    """
